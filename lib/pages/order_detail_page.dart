...
class OrderDetailPageForPickUp extends StatefulWidget {
  ...
  @override
  State<OrderDetailPageForPickUp> createState() =>
      _OrderDetailPageForPickUpState();
}

class _OrderDetailPageForPickUpState extends State<OrderDetailPageForPickUp> {
  @override
        ...
        body: SingleChildScrollView(
          child: Padding(
            ...
            child: Consumer<OrderProvider>(builder: (context, provider, child) {
              String od =
                  '${provider.pendingOrders[widget.index].hasordertype!.name}';
              ...
              return Column(
                ...
                children: [
                  ...
                  Text("${provider.pendingOrders[widget.index].orderId}",
                      style: TextStyle(
                      ...
                      child: Column(
                          ...
                          children: [
                            Text("Sender",
                                style: TextStyle(color: Colors.black54)),
                            Text(
                                "${provider.pendingOrders[widget.index].hassender!.name}",
                                style: TextStyle(fontSize: 16)),
                            Text(
                                "${provider.pendingOrders[widget.index].hasoriginBranch!.branchAddress}")
                          ]),
                    ),
                  ),
                  TimelineTile(
                      ...
                      child: Column(
                          ...
                          children: [
                            Text("Recipient",
                                style: TextStyle(color: Colors.black54)),
                            Text(
                                "${provider.pendingOrders[widget.index].hasreceiver!.name}",
                                style: TextStyle(fontSize: 16)),
                            Text(
                                "${provider.pendingOrders[widget.index].hasdestinationBranch!.branchAddress}")
                          ]),
                    ),
                  ),
                  ...
                  //Weight
                  Text(provider.pendingOrders[widget.index].hascourier!
                              .weightQuantity !=
                          null
                      ? "Weight(kg)"
                      : "Volume"),
                  Text(
                      provider.pendingOrders[widget.index].hascourier!
                                  .weightQuantity !=
                              null
                          ? provider
                              .pendingOrders[widget.index].hascourier!.actWeight
                              .toString()
                          : provider
                              .pendingOrders[widget.index].hascourier!.quantity
                              .toString(),
                      style: TextStyle(
                        fontWeight: FontWeight.bold,
                        fontSize: 16,
...
//                       )),
                  ...
                  const Text("Ordered date",
                      style: TextStyle(color: Colors.black54)),
                  Text("${provider.pendingOrders[widget.index].date}",
                      style: TextStyle(
                        fontWeight: FontWeight.bold,
                        fontSize: 16,
                      )),
                  ...
                  const Text("Order type",
                      style: TextStyle(color: Colors.black54)),
                  Text(
                      "${provider.pendingOrders[widget.index].hasordertype!.name![0].toUpperCase()}${provider.pendingOrders[widget.index].hasordertype!.name!.substring(1)}",
                      style: TextStyle(
                        fontWeight: FontWeight.bold,
                        fontSize: 16,
                      )),
                      ...
                      child: ElevatedButton(
                        onPressed: () {
                          ...
                          final snackBar = SnackBar(
                            content: Text(
                              'Order picked up. Please refresh.',
